 ...
 * "hemel_en_aarde_walker_bay", or "pinot_noir". Display-name discipline
 * everywhere — the profile is the product, not a debug view.
 *
 ...
 * or the page failed to render its profile content; both are real failures.
 */
...
test.describe("No raw internal IDs (permanent guard)", () => {
  test("home page with Full Profile expanded shows no raw IDs", async ({ page }) => {
    await page.goto("/");
    await page.getByText("Full Profile", { exact: true }).click();
    // Profile sections must actually render — a blank accordion passing the
    // scan would be outcome-tolerance in disguise
    await expect(page.getByText("Varietals", { exact: true })).toBeVisible();
