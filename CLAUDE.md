...
- `node src/lib/__tests__/dnaEvolution.test.js` — 65 tests, DNA evolution pipeline (inlined mirror of resolver + engine, incl. Suite 9 quiz-merge + uncheck invariants, Suite 10 partial-credit banding + the 10H geography-conflict guard (a conflict-capped resolution accumulates nothing, not even partial credit), Suite 11 surface flows: the /recommend accumulation path, the home double-count regression, and delete-retry idempotency)
...
- `node src/lib/__tests__/archetypeVoice.test.js` — 8 tests, the reveal-payoff guard: a SOURCE SCAN of profileEngine.js for banned deferral copy ("come back", "will grow richer/get sharper", the old "Rising Palate" name — comments included, so never quote banned copy there) + sparse-input cases through the REAL module asserting the fallback ("The Instinctive Palate") stays a confident present-tense identity (the alias-loader supplies JSON import attributes, so profileEngine needs no test mirror)
